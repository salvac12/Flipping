...
// import { AdvancedParameters } from '@/components/house-flipping/AdvancedParameters'; // Comentado temporalmente
// import PriceEstimator from '@/components/calculator/PriceEstimator'; // Comentado temporalmente - No existe
...
export default function HouseFlippingCalculatorPage() {
  ...
  // const [priceEstimation, setPriceEstimation] = useState<any>(null); // Comentado - PriceEstimator no existe
  ...
  const handleSaveAnalysis = async () => {
    // DESARROLLO: Base de datos desactivada temporalmente
    alert('Guardado desactivado (sin base de datos configurada). Los cálculos funcionan correctamente.');
    return;
  ...
  const loadSavedAnalyses = async () => {
    ...
    try {
      // DESARROLLO: Base de datos desactivada
      // const response = await fetch('/api/analysis');
      // if (response.ok) {
      //   const data = await response.json();
      //   setSavedAnalyses(data.analyses || []);
      // }
      setSavedAnalyses([]); // Sin DB, lista vacía
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      {/* Parámetros Avanzados - Comentado temporalmente
      <AdvancedParameters
      ...
      */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* PriceEstimator comentado temporalmente - Componente no existe
          <PriceEstimator
            propertyData={{
              surface: propertyData.surface,
